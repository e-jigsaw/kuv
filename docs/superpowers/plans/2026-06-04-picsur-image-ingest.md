...
- [x] **Step 2: ルート `package.json` の `onlyBuiltDependencies` に sharp を追加**
...
- [x] **Step 3: 再インストールして native build を通す**
...
- [x] **Step 4: sharp が import できるか smoke 確認**
...
- [x] **Step 1: 生成スクリプトを作成 — `packages/api/src/test/gen-fixtures.py`**
...
- [x] **Step 3: アニメ fixture が sharp で 2 ページと読めるか確認**
...
- [x] **Step 1: 失敗するテストを書く — `packages/api/src/services/filetype.test.ts`**
...
- [x] **Step 2: fixture ヘルパを作成 — `packages/api/src/test/fixtures.ts`**
...
- [x] **Step 4: 実装 — `packages/api/src/services/filetype.ts`**
...
- [x] **Step 1: 失敗するテストを書く — `packages/api/src/services/image-ingest.test.ts`**
...
- [x] **Step 3: 実装 — `packages/api/src/services/image-ingest.ts`**
...
- [x] **Step 1: ルートを実装 — `packages/api/src/routes/image.ts`**
...
- [x] **Step 3: 統合テストを書く — `packages/api/src/routes/image.test.ts`**
...
- [x] **Step 1: 失敗するテストを追記 — `packages/api/src/routes/image.test.ts`**
...
- [x] **Step 3: DELETE ルートを実装 — `packages/api/src/routes/image.ts`**
...
- [x] **Step 2: ワークスペース全体の build + typecheck**
...
全 8 タスク完了（`0166bca`〜`1ffaf98`）。45 テスト / `pnpm -r build` / `pnpm -r typecheck` 全緑。最終レビュー verdict: Ready to merge。

**Plan 3b-2 への引き継ぎ:**
- **dedupe 時の links の mime が `image/png` 固定**（`routes/image.ts` の dedupe 分岐）。3b-2 で `findImageById` を image_file(master) join に広げて filetype を返すか、配信側で ext 解決する。配信の `GET /i` も同じ lookup を使うので一本化すると良い。
- **dedupe 分岐は owner 未チェック**（認証済みなら誰でも既存画像のメタが返る）。単一 admin モデルでは問題ないが、マルチユーザー化する場合は要対応。
- **keep_original ON の統合テストが無い**（unit では ingest / queries 双方で検証済み）。3b-2 で settings 行を立てて POST → master+original の 2 行を `app.request` 経由で確認するテストを 1 本足すと縫い目が閉じる。
